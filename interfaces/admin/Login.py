...
def comprobarData(username, password):
    ...
    query = "SELECT adminId, adminName, email, created_at, ative_rol FROM users WHERE (email = %s) AND password = %s"
    interfaces.ConectorBD.cursor.execute(query, (username, username, hashed_password))
